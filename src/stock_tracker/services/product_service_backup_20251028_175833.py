...
class ProductService:
    ...
    async def sync_from_api_to_sheets(self, skip_existence_check: bool = False) -> SyncSession:
        ...
        try:
            ...
            # ИСПРАВЛЕНИЕ 28.10.2025: Использовать 7 дней назад для полной недели
            def get_week_start() -> str:
                """Получить дату 7 дней назад для полной недели данных"""
                today = datetime.now()
                week_ago = today - timedelta(days=ORDER_LOOKBACK_DAYS)  # 7 дней назад
                return week_ago.strftime("%Y-%m-%dT00:00:00")
            
            date_from = get_week_start()
            logger.info(f"Using date_from: {date_from} (7 days ago for full week period)")
